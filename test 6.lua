...
local function processRoom(room)
	...
	-- anti duplicate run lock per-room
	if room:GetAttribute("DecorProcessing") then
		return
	end
	room:SetAttribute("DecorProcessing", true)

	-- wait room fully load
	task.wait(0.5)

	-- anti double spawn
	if room:GetAttribute("HasDecorSpawned") then
		return
	end

	-- wait floor stable (important)
	local floor
	for _ = 1, 25 do
		floor = room:FindFirstChild("Floor", true)
	...
	if not floor then
		room:SetAttribute("DecorProcessing", false)
		return
	end

	-- wait key stable (fix main bug)
	task.wait(0.35)
	local hasKey = room:FindFirstChild("KeyHitbox", true) ~= nil

	local shouldSpawn = hasKey or (math.random(1, 100) <= 5)
	...
	for _, part in ipairs(decorModel:GetDescendants()) do
		if part:IsA("BasePart") then
			...
			if string.find(part.Name, "Fire") then
				part.CanQuery = true
				part.CanTouch = true
				task.spawn(setupFireDamageZone, part, room.Name)
			else
				part.CanQuery = false
				part.CanTouch = false
			end
	...
	decorModel.Parent = workspace

	local spawnCFrame = floor.CFrame * CFrame.new(0, (floor.Size.Y / 2) + 1, 0)
	decorModel:PivotTo(spawnCFrame * CFrame.Angles(0, math.rad(math.random(0, 360)), 0))
