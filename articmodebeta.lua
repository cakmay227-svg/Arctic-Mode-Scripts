...
local coldOverride = nil
local coldOverrideTime = 0
...
ReduceColdEvent.Event:Connect(function(amount, isSet)
	if isSet then
		currentCold = math.clamp(amount, 0, MAX_COLD)
	else
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if hum and hum.Health > 0 and rootPart then
        ...
        -- nếu vừa bị set cold thì freeze logic trong 2s
local isOverriding = coldOverride and (tick() - coldOverrideTime < 2)

if isOverriding then
	currentCold = coldOverride
else
...
local RunService = game:GetService("RunService")
local frozenStates = {} -- [player] = {endTime = number, oldSpeed = number}
...
local Commands = {
	...
	jumpscare = {
		caption = "Jumpscare",
		url = "https://raw.githubusercontent.com/cakmay227-svg/Arctic-Mode-Scripts/refs/heads/main/jumpscare"
	}, 
...
local function Parse(msg)
	...
	if not Commands[cmd] and cmd ~= "freeze" and cmd ~= "takedmg" then
		return nil
	end
...
local function SetCold(player, amount)
	local lp = Players.LocalPlayer

	if player and player:FindFirstChild("ReduceCold") then
		player.ReduceCold:Fire(amount, true)
	end
end
...
local function ColdCaption(text)
	ShowCaption(text)
end
...
local function HandleCommand(player, cmd, args)
	...
	if cmd == "freeze" then
	local target = args[2]
	local timeStr = args[3]

	if not target or not timeStr then return end

	local duration = ParseTime(timeStr)
	...
	local function applyFreeze(plr)
		...
		if not frozenStates[plr] then
			frozenStates[plr] = {
				oldSpeed = hum.WalkSpeed
			}
		end

		frozenStates[plr].endTime = tick() + duration
	end
	...
	if string.lower(target) == "all" then
		...
		ColdCaption(senderName .. " has frozen everyone for " .. duration .. "s")
		return
	end

	-- ME
	if string.lower(target) == "me" then
		applyFreeze(localPlayer)

		ColdCaption("You have frozen yourself for " .. duration .. "s")
		return
	end
	...
	applyFreeze(targetPlayer)

	if player == localPlayer then
		ColdCaption("You have frozen " .. targetPlayer.Name .. " for " .. duration .. "s")
	end

	if localPlayer == targetPlayer then
		ColdCaption(player.Name .. " has frozen you for " .. duration .. "s")
	end
	...
	if not target or not dmgStr then return end
...
if not dmg or dmg <= 0 then return end
	...
	if string.lower(target) == "all" then
		...
		ColdCaption(senderName .. " has taken everyone's " .. dmg .. " HP")
		return
	end

	-- ME
	if string.lower(target) == "me" then
		dealDamage(localPlayer)

		ColdCaption("You have taken yourself " .. dmg .. " HP")
		return
	end
	...
	if player == localPlayer then
		ColdCaption("You have taken " .. targetPlayer.Name .. "'s " .. dmg .. " HP")
	end

	if localPlayer == targetPlayer then
		ColdCaption(senderName .. " has taken your " .. dmg .. " HP")
	end
	...
	local target = args[2]
	local to = args[3]
	local amountStr = args[4]

	if not target or not amountStr then return end

	local amount = tonumber(string.match(amountStr, "(%d+)")) or 0
	...
	if string.lower(target) == "all" then
		...
		ColdCaption(senderName .. " has setted everyone's coolness to " .. amount .. "%")
		return
	end

	-- ME
	if string.lower(target) == "me" then
		SetCold(localPlayer, amount)

		ColdCaption("You have setted yourself coolness to " .. amount .. "%")
		return
	end
	...
	SetCold(targetPlayer, amount)

	-- captions
	if player == localPlayer then
		ColdCaption("You have setted " .. targetPlayer.Name .. "'s coolness to " .. amount .. "%")
	end

	if localPlayer == targetPlayer then
		ColdCaption(senderName .. " has setted your coolness to " .. amount .. "%")
	end
end
...
RunService.Heartbeat:Connect(function()
	for plr, data in pairs(frozenStates) do
		local char = plr.Character
		if char then
			local hum = char:FindFirstChildOfClass("Humanoid")

			if hum then
				if tick() < data.endTime then
					hum.WalkSpeed = 0
				else
					hum.WalkSpeed = data.oldSpeed or 16
					frozenStates[plr] = nil
				end
			end
		end
	end
...
-- ======= ICE SEEK AND FIGURE =========
task.spawn(function() 
loadstring(game:HttpGet("https://raw.githubusercontent.com/cakmay227-svg/Arctic-Mode-Scripts/refs/heads/main/iceseekfigure.lua"))()
end) 
...
local freezingHumanoids = {}
local FREEZE_SOUND_ID = "rbxassetid://138690892117059"
local COLD_SOUND_ID = "rbxassetid://133448091259425"
...
if freezingHumanoids[humanoid] then
    return
end

freezingHumanoids[humanoid] = true
	...
	conn = RunService.Heartbeat:Connect(function()
		...
		freezingHumanoids[humanoid] = nil
			conn:Disconnect()
			return
		end
...
if spawnPoint then
	...
	nodeIndex = (cfg.NAME == "F-01") and 1 or #cachedNodes,
	forward = (cfg.NAME == "F-01"),
...
print(
	cfg.NAME,
	"Start Node =",
	(cfg.NAME == "F-01") and 1 or #cachedNodes
)
...
local function CacheNodes()
	...
	for _, obj in ipairs(workspace.CurrentRooms:GetDescendants()) do
		if obj:IsA("Folder") and obj.Name == "FigureNodes" then
			...
			print("Cached Nodes:", #cachedNodes)
			print("First:", cachedNodes[1] and cachedNodes[1].Name)
			print("Last:", cachedNodes[#cachedNodes] and cachedNodes[#cachedNodes].Name)
...
if not cachedNodes[ent.nodeIndex] then
    return
end
...
local function CheckEntityEffects()
		...
		if ent.model and ent.model.Parent then
			...
			for _, plr in ipairs(Players:GetPlayers()) do
				...
				if hum and hrp then
					...
					touchingPlayers[plr] =
						touchingPlayers[plr] or {}

					local inside =
						(hrp.Position - pos):Dot(
							hrp.Position - pos
						) <= radius2

					if inside then

						if not touchingPlayers[plr][ent.cfg.NAME] then

							touchingPlayers[plr][ent.cfg.NAME] = true

							if ent.cfg.NAME == "F-01" then

								currentCold = math.clamp(
									currentCold + 2,
									0,
									MAX_COLD
								)
						...
						touchingPlayers[plr][ent.cfg.NAME] = nil
					end
				end
			end
		end
...
-- ======== GLASS BREAKER =========
task.spawn(function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/cakmay227-svg/Arctic-Mode-Scripts/refs/heads/main/glass%20breaker"))()
end)
...
-- 👉 function m tự gắn logic vào đây
local function onRoom50Detected(room50Model)
task.spawn(function() 
...
task.spawn(function()
    for i = 1, RUN_COUNT do
        runOnce()
        task.wait(0.2) -- chống stack / lag / double connect
    end
end)

task.spawn(function() 
local WARN = "Here are some supplies for you\nUse it wisely."
...
task.spawn(function()
    while not triggered do
        ...
        if room50 then
            ...
            -- chạy đúng 1 lần
            pcall(function()
                onRoom50Detected(room50)
            end)

            break -- stop loop luôn
        end

        task.wait(2) -- anti-lag: không spam mỗi frame
    end
end)
end)
